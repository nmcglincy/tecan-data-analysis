...
# 
# All looks good, but evidently the values of lambda are noisy, and don't really reflect what I see
# Adding summary mean and se
head(banana)
banana.summ = ddply(banana,
...
ggplot(banana, aes(x = sample, y = measurement)) +
  geom_crossbar(data = banana.summ, aes(x = sample, y = mean, ymin = mean-se, ymax = mean+se), size = 0.25) +
